...
function Home() {
  ...
  useEffect(() => {
    fetch('http://localhost:3000/movies')
      ...
      .then(data => {
        console.log(data)
        const formatedData = data.data.results.map(movie => {
        ...
        setDiscoverMovies(formatedData);
      });
  }, []);
  ...
  const movies = discoverMovies.map((data, i) => {
    ...
    return <Movie key={i} updateLikedMovies={updateLikedMovies} isLiked={isLiked} title={data.title} overview={data.overview.slice(0, 250)+ "..."} poster={data.poster} voteAverage={data.voteAverage} voteCount={data.voteCount} />;
  });
